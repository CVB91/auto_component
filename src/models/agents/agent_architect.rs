...
impl AgentSolutionArchitect {
    ...
    async fn call_project_scope(&mut self, factsheet: &mut FactSheet) -> ProjectScope {
        ...
        let ai_response: ProjectScope = ai_task_request_decoded::<ProjectScope>(
            ...
            get_function_string!(print_project_scope), // This is a macro which turns print_project_scope into a string and then also prints that to the terminal
            print_project_scope,
        )
        .await;
...
impl SpecialFunctions for AgentSolutionArchitect {
    ...
    async fn execute(
        ...
        // !!! WARNING - BE CAREFUL OF INFINATE LOOPS !!!
                ...
                AgentState::UnitTesting => {
                    ...
                    if exclude_urls.len() > 0 {
                        let new_urls: Vec<String> = factsheet
                            ...
                            .unwrap()
                            .iter()
                            .filter(|url: &&String| !exclude_urls.contains(&url))
                            .cloned()
                            .collect();
